// client/src/pages/Laporan/LapPerawatanTanaman.js
import React, { useState, useEffect } from 'react';
import { getPerawatanTanamans } from '../../services/perawatanTanamanService'; // Panggil dari service yang sudah terintegrasi Axios
...
function LapPerawatanTanaman() {
  ...
  useEffect(() => {
    const fetchLaporan = async () => {
      ...
      setError(null);
      try {
        // Panggil fungsi getPerawatanTanamans dari service
        const responseData = await getPerawatanTanamans(selectedLahanId); 
  ...
  const exportPdf = () => {
    ...
    doc.text(`Laporan Perawatan Tanaman ${selectedLahanId ? `(Lahan: ${lahans.find(l => l._id === selectedLahanId)?.nama})` : ''}`, 14, 20);
    ...
    doc.save(`Laporan_Perawatan_Tanaman_${selectedLahanId ? lahans.find(l => l._id === selectedLahanId)?.nama : 'Semua'}.pdf`);
  };
  ...
  const exportCsv = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Laporan Perawatan Tanaman");
    const wbout = XLSX.write(wb, { bookType: 'csv', type: 'array' });
    saveAs(new Blob([wbout], { type: 'application/octet-stream' }), `Laporan_Perawatan_Tanaman_${selectedLahanId ? lahans.find(l => l._id === selectedLahanId)?.nama : 'Semua'}.csv`);
  };

  return (
    <Box sx={{ p: 3 }}>
      <Typography variant="h4" gutterBottom>
        Laporan Perawatan Tanaman
      </Typography>
      ...
      {loading ? (
        ...
        <Typography>Tidak ada data perawatan tanaman untuk ditampilkan.</Typography>
      ) : (
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label="laporan perawatan tanaman table">
            <TableHead>
              <TableRow>
...
export default LapPerawatanTanaman;