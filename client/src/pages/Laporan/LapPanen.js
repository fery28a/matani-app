// client/src/pages/Laporan/LapPanen.js
import React, { useState, useEffect } from 'react';
import { getPanens } from '../../services/panenService'; // Panggil dari service yang sudah terintegrasi Axios
...
function LapPanen() {
  ...
  useEffect(() => {
    const fetchLaporan = async () => {
      ...
      setError(null);
      try {
        // Panggil fungsi getPanens dari service
        const responseData = await getPanens(selectedLahanId); 
  ...
  const exportPdf = () => {
    ...
    doc.text(`Laporan Panen ${selectedLahanId ? `(Lahan: ${lahans.find(l => l._id === selectedLahanId)?.nama})` : ''}`, 14, 20);

    const tableColumn = ["No.", "Lahan", "Tanggal", "Deskripsi", "Kebutuhan", "Pekerja", "Total Biaya"];
    const tableRows = [];
    ...
    doc.save(`Laporan_Panen_${selectedLahanId ? lahans.find(l => l._id === selectedLahanId)?.nama : 'Semua'}.pdf`);
  };

  const exportCsv = () => {
    const header = ["No.", "Lahan", "Tanggal", "Deskripsi", "Kebutuhan", "Pekerja", "Total Biaya"];
    ...
    saveAs(new Blob([wbout], { type: 'application/octet-stream' }), `Laporan_Panen_${selectedLahanId ? lahans.find(l => l._id === selectedLahanId)?.nama : 'Semua'}.csv`);
  };
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      {loading ? (
        ...
        <Typography>Tidak ada data panen untuk ditampilkan.</Typography>
      ) : (
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label="laporan panen table">
            <TableHead>
              <TableRow>
