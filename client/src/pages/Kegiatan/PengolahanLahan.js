// client/src/pages/Kegiatan/PengolahanLahan.js
import React, { useState, useEffect } from 'react';
// Import SEMUA fungsi CRUD dari service
import { getPengolahanLahans, createPengolahanLahan, updatePengolahanLahan, deletePengolahanLahan } from '../../services/pengolahanLahanService';
...
// Import komponen Material-UI yang akan digunakan
import {
  TextField,
  Button,
  Table,
  ...
  FormControl,
  IconButton,
  List,         // <--- Digunakan untuk menampilkan item multi-list di tabel
  ListItem,     // <--- Digunakan untuk menampilkan item multi-list di tabel
  ListItemText  // <--- Digunakan untuk menampilkan item multi-list di tabel
} from '@mui/material';

// Import ikon Material-UI yang akan digunakan
...
function PengolahanLahan() {
  // Pastikan semua state yang didefinisikan digunakan di JSX atau fungsi
  ...
  const fetchKegiatans = async () => {
    ...
    setError(null);
    try {
      const responseData = await getPengolahanLahans(); // Mengambil data melalui service
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        const kegiatanResponse = await getPengolahanLahans(); // Mengambil data melalui service
        setKegiatans(kegiatanResponse.data);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingId) {
        await updatePengolahanLahan(editingId, kegiatanData); // <--- Menggunakan fungsi service
        alert('Data pengolahan lahan berhasil diperbarui!');
      } else {
        await createPengolahanLahan(kegiatanData); // <--- Menggunakan fungsi service
        alert('Data pengolahan lahan berhasil ditambahkan!');
      }
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Apakah Anda yakin ingin menghapus kegiatan pengolahan lahan ini?')) {
      setError(null);
      setLoading(true);
      try {
        await deletePengolahanLahan(id); // <--- Menggunakan fungsi service
        alert('Kegiatan pengolahan lahan berhasil dihapus!');
        fetchKegiatans();
        if (editingId === id) {
          resetForm();
        }
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ display: 'flex', flexDirection: 'column', gap: 2, mb: 4, maxWidth: 800 }}>
        ...
        <TextField
          ...
          required
          InputLabelProps={{ shrink: true }} // Penting untuk type="date"
        />
        ...
        <TextField
          ...
          variant="outlined"
          fullWidth
          multiline // Mengizinkan banyak baris
          rows={3}
          value={deskripsi}
          onChange={(e) => setDeskripsi(e.target.value)}
          placeholder="Contoh: Pembajakan lahan, pembuatan bedengan"
        />
      ...
      <Typography variant="h5" gutterBottom sx={{ mt: 4 }}>
        Daftar Kegiatan Pengolahan Lahan
      </Typography>

      {kegiatans.length === 0 && !loading ? (
        <Typography>Belum ada kegiatan pengolahan lahan yang ditambahkan.</Typography>
      ) : (
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 650 }} aria-label="pengolahan lahan table">
            <TableHead>
              <TableRow>
...
export default PengolahanLahan;