// server/models/PengolahanLahan.js
const mongoose = require('mongoose');

const pengolahanLahanSchema = mongoose.Schema(
  {
    lahan: {
    ...
    pupukDigunakan: [ // Array objek untuk pupuk yang digunakan
      {
        pupuk: {
        ...
        // Harga per item akan diambil dari data pupuk master saat ditampilkan/dihitung
      },
    ],
...
pengolahanLahanSchema.pre('save', async function(next) {
  let calculatedTotalBiaya = 0;

  // Pastikan referensi pupuk, kebutuhan, pestisida sudah di-populate jika ini dari update
  // atau ambil langsung dari database jika baru dibuat/diperbarui
  ...
  for (const item of this.pestisidaDigunakan) {
    ...
    if (pestisidaDoc) {
      // Hitung harga per satuan kecil dari pestisida master, lalu kalikan jumlah yang digunakan
...
const PengolahanLahan = mongoose.model('PengolahanLahan', pengolahanLahanSchema);

module.exports = PengolahanLahan;